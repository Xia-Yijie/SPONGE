...
//////////////////////////////
void Particle_Mesh_Ewald::Initial(CONTROLLER *controller, int atom_numbers, VECTOR boxlength,char *module_name)
{
	if (module_name == NULL)
	{
	...
	else
	{
		...
		cutoff = 10.0;
		if (controller[0].Command_Exist("cut"))
			cutoff = atof(controller[0].Command("cut"));

		tolerance = 0.00001;
		if (controller[0].Command_Exist("PME_Direct_Tolerance"))
			tolerance = atof(controller[0].Command("PME_Direct_Tolerance"));

		fftx = -1;
		ffty = -1;
		fftz = -1;
		if (controller[0].Command_Exist("fftx"))
			fftx = atoi(controller[0].Command("fftx"));
		if (controller[0].Command_Exist("ffty"))
			ffty = atoi(controller[0].Command("ffty"));
		if (controller[0].Command_Exist("fftz"))
			fftz = atoi(controller[0].Command("fftz"));
